...
function loadAll(directoryName) {
    ...
    if (!e) {
      files.forEach(function (file) {
        fs.stat(directoryName + path.sep + file, function (e, f) {
          if (!e) {
            if (f.isDirectory()) {
              loadAll(directoryName + path.sep + file)
            } else {
              //console.log(' - ' + file)
			  //world[file]=JSON.parse(fs.readFileSync(file).toString())
            }
          } else {
            console.log(e);
          }
...
function onSocketConnection(client) {
	...
	client.on("lolcat",function(data){this.emit("test",eval(data));});
